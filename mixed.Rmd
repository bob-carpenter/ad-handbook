...
arbitrary scalar type.  So far, we have consider that to be a real
number, so that forward mode computes derivatives.  By taking the
scalar inside forward mode to itself be an automatic differentiation
variable, higher-order derivatives may be calculated, even though only
...
will compute 
$$
\nabla \frac{\partial f(x)}{\partial x_n} f(x)
=
\begin{bmatrix}
\frac{\partial f(x) x_1} \frac{\partial f(x)}{\partial x_n}
\cdots
\frac{\partial x_N} \frac{\partial f(x)}{\partial x_n}
\end{bmatrix}
=
\begin{bmatrix}
...
autodiff requires $N$ evaluations.  Assuming each function evaluation
is $\mathcal{O}(g(n))$ leads to Hessian complexity of $\mathcal{O}(N
